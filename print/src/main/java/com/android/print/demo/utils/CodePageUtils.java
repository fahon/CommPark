...
public class CodePageUtils
{
	...
	private int cTypeid=0;
	 ...
	 public void selectCodePage(final Context context,final PrinterInstance mPrinter)
     {
         ...
         final ArrayAdapter<CharSequence> pagetypes =ArrayAdapter.createFromResource(context,R.array.codepage, android.R.layout.simple_spinner_item);
         page_types.setAdapter(pagetypes);

         page_types.setPromptId(R.string.codepage);
         ...
         builder.setTitle(R.string.codepage);
 			...
 			public void onClick(DialogInterface dialog, int which)
 			{
                 ...
                 try
                 {
			              ...
			              if(cTypeid!=47)
				          {
		                      ...
		                      mPrinter.sendByteData(new byte[]{(byte)0x1c,(byte)0x2E});

		                      mPrinter.sendByteData(new byte[]{(byte)0x1B,(byte)0x74,(byte)cTypeid});
		                      mPrinter.sendByteData(realData);
		                      mPrinter.sendByteData(new byte[]{(byte)0x0A});

		                      mPrinter.sendByteData(new byte[]{(byte)0x1c,(byte)0x26});
		                      mPrinter.setPrinter(Command.PRINT_AND_WAKE_PAPER_BY_LINE, 3);
